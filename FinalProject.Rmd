...
-   The 5th member had a foot injury and served as the designated time keeper, using a stopwatch to record lap times.
...
\
The Residuals v Fitted plot is used to check the assumption of homoscedasticity, that the variance of the errors is constant across all predicted values.
The residuals are fairly evenly scattered around the horizontal zero line, indicating that the model does not show evidence of heteroscedasticity, which is necessary for the model to work properly.
The normal Q-Q Plot allows us to see that the residuals are normally distributed, which we need for ANOVA. Here, it's close enough, given that our dataset is only 16 observations. These two tests are just key checks to ensure that ANOVA is running smoothly.\
\
