...
-   More specifically, we want to see whether the time it takes to run around the track significantly differs when different people run in **sneakers, flip-flops, running shoes, or barefoot.**

**Context/Motivation**

-   Footwear is a big part of everyday life and sports performance. Runners often invest in specialized shoes, but regular people also sometimes run in casual shoes or even barefoot. We were interested in whether these different footwear choices actually lead to **measurable differences in running speed** over a fixed distance of a half mile around the track. Our experiment uses a simple 400 m track setting with repeated timed runs to compare running performance across four different footwear conditions.
...
Each observation in our dataset is two timed laps (1/2 mile) around the track under one of these four footwear conditions
...
-   The design can be described as a 4x4 Latin Square

    -   A Latin Square is a row-column design experiment where the number of rows (m levels of one nuisance factor) = number of columns (n levels of another nuisance factor) = number of treatments. This is to effectively create (m \* n) blocks at each different combination of levels to attempt to average out the variability created by both of the identified nuisance factors.
...
-   We used a 4x4 Latin Square design (Below) with 4 different days and 4 different runners as the blocking factors and used 4 different footwear types as the primary treatment factor.
...
-   **Randomization:**
    ...
    -   The order of footwear across days was randomized for each individual such that no particular footwear was always run on the "best" or "worst" day.
...
-   **Total Observations**: 4\*4 = 16

# Data Collection

**Procedure**:

-   We conducted the experiment on the outdoor track on campus over 4 separate days (Sunday, November 23 - Wednesday, November 26)

-   On each day, the same 4 group members who were able to run came to the track.

-   The 5th member had a foot injury and served as the designated time keeper, using a stopwatch to record lap times. This also served to reduce variation in human error as opposed to having multiple people time laps.

-   At the start of each day, we each put on the randomly assigned footwear we were assigned for that specific day, according to the predetermined 4x4 Latin Square.
...
-   Not everyone had the exact same shoe - we made sure the type of shoes were the same, but they were not brand-specific, which could lead to deviation.

-   Because we were concerned about fatigue and changing weather, we spread the data collection across four days and treated each day as a block

    -   Some scheduling issues led to a time delay - a real-world effect on experiment efficiency.
...
We are using a fixed-effects ANOVA model for a $4 \times 4$ Latin Square Design. The linear model is, $Y_{ijk} = \mu + \tau_i + \beta_j + \gamma_k + \epsilon_{ijk}$. Where $\mu$ is the grand mean, $\tau_i$ is the fixed footwear effect, $\beta_j$ is the runner effect, $\gamma_k$ is the day effect and $\epsilon_{ijk}$ is the random error.

The hypotheses are:

-   $H_0: \tau_{Sneakers} = \tau_{Barefoot} = \tau_{FlipFlops} = \tau_{RunningShoes} = 0$

-   $H_a:$ At least one footwear type results in a different mean running time.
...
kable(tukey_shoes,
      caption = "Tukey HSD Pairwise Comparisons for Footwear",
      digits = 4)
```
...
plot(ls_model, which = 1,
main = "Residuals vs Fitted Values")
plot(ls_model, which = 2,
main = "Normal Q–Q Plot of Residuals")

#Assumption testing
shapiro_test <- shapiro.test(residuals(ls_model))
bartlett_test <- bartlett.test(Time ~ Shoe, data = analysis_df)
...
\
Looking at the box plots, we can see the reasoning behind the hypothesis test much more easily. Looking first at the shoe box-plot, we see that the runners, on average, ran the fastest in running shoes, followed by sneakers, barefoot, and finally flip-flops. The flip-flops had a much higher variance than the other shoes, which could be due to the fact that flip-flops are not designed for running and could lead to inconsistent performance. The visual differences, however, are not enough to achieve a significant result, as we saw from the ANOVA, showing us truly how powerful a $\alpha = .05$ hypothesis test can be. Switching gears to the runner box-plot, we can clearly see that there is a lot of variability between runners. Some runners are just naturally faster and more consistent than others, which is why blocking for a runner was a good idea. This visual difference was enough to achieve significance in our ANOVA test.

Although the global ANOVA F-test was non significant, we conducted a Tukey's HSD post-hoc analysis to find which specific pairwise differences contributed to this result. The Tukey intervals showed that the difference between Running Shoes and Flip Flops was the biggest, with a difference of 37.75 seconds and an adjusted p-value of 0.04. If our sample size had been larger, this difference may have made the global F-test significant. \
\
The Residuals v Fitted plot is used to check the assumption of homoscedasticity, that the variance of the errors is constant across all predicted values. The residuals are fairly evenly scattered around the horizontal zero line, indicating that the model does not show evidence of heteroscedasticity, which is necessary for the model to work properly. On top of this, Bartlett's test for homogeneity of variances resulted in a p-value of 0.24, which confirms the visual evidence from the residual vs. fitted plot. The normal Q-Q Plot allows us to see that the residuals are normally distributed, which we need for ANOVA. Here, it's close enough, given that our dataset is only 16 observations. The Shapiro-Wilk test for normality of residuals resulted in a p-value of 0.66, consistent with the Normal Q-Q Plot. These four tests are just key checks to ensure that ANOVA is running smoothly.\
\
One of the drawbacks of this experiment was the different shoes runners were wearing. Although every runner ran with each "type" of footwear, the specific brand and model of each shoe was not controlled for. This could have led to some runners having better or worse shoes than others, which could have affected their times; this could have played a part in our ANOVA not being significant for the shoes.\
Another drawback of our design is that we only obtained one observation per (runner, day, footwear) cell in the 4x4 Latin square, due to limited time and the physical demands of our experiment. This means we cannot separate interaction effects from random error, and any unusual run directly inflates the error term. With only one observation per cell and few error degrees of freedom, our estimate of variability is unstable, and the ANOVA has lower power to detect real differences between footwear types.
