...
\
The Residuals vs. Fitted plot is used to check the assumption of homoscedasticity, that the variance of the errors is constant across all predicted values.
...
The normal Q-Q Plot allows for us to see that the residuals are normally distributed, which we need for Anova. Here, it's close enough, given that our dataset is only 16 observations. These two tests are just key checks to ensure that Anova is running smoothly.\
\
